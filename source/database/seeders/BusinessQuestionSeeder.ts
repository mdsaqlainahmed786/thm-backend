...
import { HomeStays, Restaurants, MarriageBanquets, BarsClubs, } from "./BusinessSubtypeSeeder";
import BusinessQuestion from "../models/businessQuestion.model";
class BusinessQuestionSeeder {
    ...
    async shouldRun() {
        const count = await BusinessQuestion.countDocuments().exec();
        return count === 0;
    }
    async run() {
        const questions = [
        ...
        const questionData = await Promise.all(
            questions.map(async (question) => {
                ...
                const businessQuestion = await BusinessQuestion.findOne({ businessTypeID: { $in: businessTypeID }, businessSubtypeID: { $in: businessSubtypeID }, question: question.question });
                if (!businessQuestion) {
...
export default BusinessQuestionSeeder;